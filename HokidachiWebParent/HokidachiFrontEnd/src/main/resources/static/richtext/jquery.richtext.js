       ...
       */
        ...
        function convertCaretPosition(editorID, selection, reverse) {
            ...
            for (var i = 0; i < code.length; i++) {
                if (code[i] === "<") {
                    // HTML tag starts
                    districts.isTag = true;
                    districts.tag = false;
                    districts.tagsCount++;
                } else if (districts.isTag === true && code[i] !== ">") {
                    districts.tagsCount++;
                } else if (districts.isTag === true && code[i] === ">") {
                    districts.isTag = false;
                    districts.tag = true;
                    districts.tagsCount++;
                } else if (districts.tag === true) {
                    districts.tag = false;
                }

                if (!reverse) {
                    if ((selection.start + districts.tagsCount) <= i && districts.isHighlight && !districts.isTag && !districts.tag && !districts.start) {
                        selection.start = i;
                        districts.start = true;
                    } else if ((selection.start + districts.tagsCount) <= i + 1 && !districts.isHighlight && !districts.isTag && !districts.tag && !districts.start) {
                        selection.start = i + 1;
                        districts.start = true;
                    }
                    if ((selection.end + districts.tagsCount) <= i + 1 && !districts.isTag && !districts.tag && !districts.end) {
                        selection.end = i + 1;
                        districts.end = true;
                    }
                }
